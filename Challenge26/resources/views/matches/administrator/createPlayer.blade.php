...
<main>
    ...
    <div class="bg-white border border-light-subtle rounded border-2 w-50 mx-auto mt-5">
        <h4 class="border-bottom border-light-subtle p-3 bg-light fs-5">Create new Player</h4>

        <form action="{{ route('player.create') }}" method="POST" class="p-3">
            @csrf
            <div class="mb-3">
            ...
            <div class="my-3">
                ...
                <select name="team_id" id="team_id" class="form-control">
                    ...
                    <option value="{{ $team->id }}" {{ $team->id === old('team')  ? 'selected' : '' }}>{{ $team->name }}</option>
                    @endforeach
                </select>
                @error('team')
                <small class="text-danger fw-bold d-block">{{ $message }}</small>
